...
- Set media AFK - Reply to a photo, GIF, or sticker with `/afk` or `brb` . Your media will be shown to people who mention you 

**🔔 WHAT HAPPENS WHEN YOU'RE AFK?**
✅ Someone mentions you (@username) . They see your AFK reason & how long you've been away !
...
async def afk_handler(_, message: Message):
    ...
    if verifier:
        ...
        try:
            ...
            base_text = f"**{message.from_user.first_name}** is now available again after  {seenago}"
            ...
            sent_msg = await message.reply_text(
                f"**{message.from_user.first_name}** is active again",
                disable_web_page_preview=True,
            )
            await track_message_for_deletion(sent_msg)
        return
